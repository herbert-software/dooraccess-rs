...
//!
//! 四个确定性协议核心模块。
//! httpx bare HTTP/1.1 + JSON encoder + 控制面/元信息。
//! 监听器（listen6672 / listen18022 PF_PACKET 抓帧）+ wire TCP sender + unlock 核心。
//!
//! ## crate gate 破口边界
//!
//! 协议核心 + HTTP 层守 0-crate（仅 std）。监听器层把依赖**放宽为 std + libc only**——
...
pub mod config;
pub mod control;
/// 并发骨架：Job 队列 / 单 wire-worker / shutdown / detached push 排空。
pub mod daemon;
pub mod ha_push;
pub mod httpx;
pub mod info;
/// 中央带时间戳日志层：`format_log_line`/`render`/`log_line`。
...
/// ExecuteUnlockFromRing + auto-hangup detached 计时线程）。
pub mod self_unlock;
pub mod sender;
/// 视频转发栈：RTP 解析 / 专有分片重组 / FLV transmux 等。
pub mod video;
pub mod wire18022;

// --- FFI / BPF 地基 ---
/// 烤好的 BPF filter 常量（预编译的 cBPF 指令序列）。
pub mod bpf;
