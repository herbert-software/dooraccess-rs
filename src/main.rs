//! `dooraccess-rs` daemon 编排入口。
//!
//! 两个主函数 `main_impl` + `run`：
//!   - `main_impl(args, stderr) -> exitcode` 可测包装：解析 `--config` flag、加载 config、
//!     缺失/废弃/解析告警逐条 log、退出码区分（flag 错=2 / config 错=1）。
//!   - signal：SIGHUP 显式忽略 + SIGTERM/SIGINT 注册。
//!   - `run`：构造并发骨架（[`daemon`] 模块的 wire-worker + Job 队列 + 单 `Arc<AtomicBool>`
//!     shutdown + [`PushTracker`] 排空），阻塞到 SIGTERM/SIGINT，执行钉死 shutdown 排序。
//!
//! passive-shadow 探针迁至 `examples/probe.rs`（`cargo build --example probe`），逻辑零回归。
...
// 中央带时间戳日志入口：视频 SharedLogFn / 散落 eprintln 收编经它。
...
/// automation flag 持久 state 文件路径。必落 /etc
...
/// `main` 的可测包装：参数化 args 与 stderr sink，返退出码。
///
/// 退出码：
///   - flag 解析失败 → **2**（参数错误）
///   - config 加载失败 → **1**（业务失败）
...
///   - 干净退出 → **0**
///
/// flag/config 失败均**不启动任何 listener/worker/HTTP 线程**；flag 错与 config 错退出码区分。
pub fn main_impl<W: Write>(args: &[String], stderr: &mut W) -> i32 {
    ...
    // ④ hass.token 配但 hass.api 缺 → log 并继续（不退出）。
...
/// `Vec<u8>` sink 测试断言落空。两者共享 `format_log_line` 渲染，各写各的 sink。
...
// ---------------------------------------------------------------------------
// signal 处理（std + libc only：忽略 SIGHUP / 捕获 SIGTERM,SIGINT）
// ---------------------------------------------------------------------------
...
/// daemon 须对 SIGHUP 不响应）；SIGTERM/SIGINT → 置位 [`SIGNAL_SHUTDOWN`]。
...
/// run 构造并发骨架，阻塞到 SIGTERM/SIGINT，执行钉死 shutdown 排序。
///
/// 接线范围：共享依赖构造（Sender/HaPushClient/slaves）+ listener（6672/18022）
...
/// push（automation_state + diagnosis）+ 手动 unlock 经 worker、号码查询 handler、fatal 上报。
///
/// 钉死 shutdown 排序（graceful shutdown）：
...
fn run<W: Write>(
    ...
    // detached push（仅取消；超时由 InstantDeadline 另行承载）。
    let shutdown = Arc::new(AtomicBool::new(false));

    // detached push 排空集（std-native pushWG 等价）。
    let push_tracker = PushTracker::new();

    // ── automation flag 启动加载优先级（state>config>env）──
    let (auto_unlock0, auto_hangup0, flag_source) =
        load_automation_flags(state_path, cfg, &mut |m| logf(stderr, m));

    // 运行时可变两 flag。单一 state：HTTP server 拿一个 share() handle 拨动，
    // Persister value_source 拿另一个 share() handle 读运行时真值——两 handle 背后同一对
    // Arc<AtomicBool>，故 endpoint 拨动后 persister 立即读到翻转值（不分叉，单一 state 注入两处）。
    let automation = AutomationState::new(auto_unlock0, auto_hangup0);

    // Persister：endpoint 拨动后 best-effort 原子落盘。
    // value_source 读运行时 atomic 当前真值。
    let persister = {
    ...
    // ── 视频转发子系统：仅 cfg.video.forward=true
    ...
    // slaves 解析：显式 iface_list 优先，否则桥成员枚举（resolve_iface_list）。
    ...
    // ── listen18022 Listener 构造（OnDetect 每帧 FormatLog + req=704 门铃 push）──
    // 先构造（不起线程），以便把同一 Arc 既给 wire-worker 作 bye 早停订阅源（listener
    // 注入），又给 listen18022 线程跑 I/O。slaves 空时 None（不启动）。
    ...
    // ── Job 队列 + 单 wire-worker 线程 ──
    // worker 跑 wire-bound Job::Unlock（free fn unlock::execute_unlock）；Job::Shutdown
    // 哨兵 break。注入真 WireSenderAdapter（sender.rs，实现 UnlockWire）+
    // listen18022 作 bye 早停订阅源。
    ...
    // auto-hangup 单帧 wire 出站缝（worker 发 req=708 preview-stop）。注入
    ...
    // ── listener 线程接线（复用已 hAP 真机认证的 listen6672/listen18022，I/O 零改动）──
    ...
    // ── self-unlock 消费者线程（ring→消费者→Job::Unlock）──
    ...
    // listen6672：号码查询 callback 安装 + 起线程（on_ring=None——
    // ring 触发 self-unlock 由 listen18022 消费者承担，此处不消费 ring 帧产 Unlock job）。
    if !slaves.is_empty() {
    ...
    // ── HTTP 8080 server 接线（复用 httpx + control handlers）──
    // 注入 Pusher（detached spawn_push）+ Persister hook + Automation + Sender。
    // control handler 拨动 flag 后调 persister.persist()（经 FnPersistHook 接线）。
    // 手动 unlock 经 wire-worker 派发（见下）。
    ...
    // 手动 unlock 经 wire-worker 队列：HTTP /unlock handler 投 Job::Unlock + 等
    ...
    // ── banner（automation flag 加载之后渲染部署参数 + flag 最终值与来源）──
    ...
    // ── startup push（automation_state + diagnosis；均 banner 之后、detached）──
    ...
    // 阻塞到 SIGTERM/SIGINT，或 HTTP 线程提前结束（bind 失败等 fatal——HTTP 是必备控制
    // 面，其线程早退即视为 fatal，不等信号、走 shutdown 排序后非零退出码）。listener 线程提前
    // 退出**不**进此条件（listener 失败非致命，HTTP-only 模式继续）。
    loop {
    ...
    // ①v VideoMgr shutdown（带 deadline 5s；video 未启用 = no-op）——
    //    **先于 join listener/HTTP**：video
    ...
    //    deadline 内完成（反序必卡满 timeout）。
    if let Some(mgr) = &video_mgr {
        mgr.shutdown(std::time::Duration::from_secs(5));
    }

    // ② 先 join listener 线程（各于下个 SO_RCVTIMEO ~500ms wakeup 退出）。
    //    join 完即无新 OnDetect → 无新门铃 push spawn（关竞态窗口）。
    for t in listener_threads {
        let _ = t.join();
    }

    // ②.5 join self-unlock 消费者线程：listener-join
    ...
    //    ④ 须先于 ⑤（所有在飞 handler 结束后再 join push，否则晚到 handler push 越过 ⑤）。
    ...
    // fatal 上报：HTTP 线程返 Err（bind 失败 / serve 非 graceful 错）→ run 返 Err →
...
/// 真 wire 注入：wire-worker 跑 unlock job 时单次三步握手用的
...
// ---------------------------------------------------------------------------
// DetachedPusher：control.rs Pusher → daemon spawn_push（业务 push off-worker）
// ---------------------------------------------------------------------------

/// 把 control.rs 的同步 `Pusher::push` 适配为 detached `spawn_push`（手动 unlock 的
/// business-result push 走 detached 线程，off wire-worker）。
...
// `dooraccess_rs::orchestration`，使 e2e 测真生产函数。main.rs 只剩薄
// 入口 + 线程编排。
// ---------------------------------------------------------------------------

/// 渲染启动 banner（部署参数）到 syslog（info.rs 仅 `build`，故 banner 行在此组装——
/// 一眼看完部署参数）。
...
/// bool → "true"/"false" String（automation_state push 字段编码）。
...
mod tests {
    ...
    fn main_impl_config_missing_returns_1() {
        ...
        // load config 路径经 logf 收编进注入的 W sink。
        assert!(out.contains("load config"), "应 log config 加载失败: {out}");
        // 断 load config 路径的输出行带时间戳前缀 shape `^dooraccess-rs: \d{4}/`，使收编机械可验。
