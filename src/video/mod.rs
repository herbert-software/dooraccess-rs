//! video：Go `internal/video/` 视频转发栈的 Rust 移植（`port-rust-video-forward`）。
//!
//! 协议真相锚 `openspec/specs/anjubao-video-stream/`（sealed）与
//! `samples/anjubao-video-stream/observations.md`；Go v0.10.0 为行为级 + 字节级
//! 等价对照基线。crate gate 仅 std + libc；MIPS32 红线禁 `AtomicU64`/`AtomicI64`。
//!
//! 子模块：
...
//!     不经骨架 wire-worker——授权见骨架 MODIFIED delta）
//!   - [`rtcp`]：RR/SDES/BYE 构造 + 5s 周期 keepalive sender（CNAME 字面
//!     `"dooraccess-go"`，design D7 golden 纪律禁改名）
//!   - [`transmux`]：FLV 静态构造（header / AVCDecoderConfigurationRecord /
//!     seq-header tag / NALU tag）+ `StreamWriter`（订阅循环 / ts 换算 /
//!     per-tag flush）
//!
...
//!
//! HTTP endpoint（`/video/*`）与 main.rs 编排接入由组 F 追加。
//!
//! req=704/708 preview 帧 builder 在 [`crate::wire18022`]（design D1：与
