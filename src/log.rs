//! 中央带时间戳日志层。
//!
//! 此前无中央日志层：`main.rs:logf`、视频 `SharedLogFn`、散落 `eprintln!` 都只输出
//! `dooraccess-rs: <msg>`，**不带任何墙钟时间戳**（进程内只有 `Instant::now()` 单调钟，
//! 无法格式化为日期）。本模块补上进程内自带的墙钟时间戳，日志行格式为
//! `dooraccess-rs: YYYY/MM/DD HH:MM:SS.mmm <msg>`。
//!
//! ## 设计要点
//!
...
//! - [`render`] 是**纯函数**（只读 `tm` 字段做 `format!`，不调 libc、不读环境）——确定性
...
/// 确定性单测用手工构造的 `tm` 调用（把「时间格式正确性」从 TZ 全局态解耦）。
///
/// 格式（毫秒精度）：`dooraccess-rs: YYYY/MM/DD HH:MM:SS.mmm <msg>`。`tm_year` 是
/// 「1900 起的偏移」、`tm_mon` 是 0-11，故 `+1900` / `+1`；`{:03}` 零填充覆盖 7ms→`.007`。
...
pub fn format_log_line(now: SystemTime, msg: &str) -> String {
    ...
    let local_tm = if secs_in_time_t_range::<libc::time_t>(secs) {
        ...
        // localtime_r 内部按需初始化 tz（不显式 tzset），MT-safe。
...
/// null 降级决策接缝。**纯函数**（除占位 `zeroed` 外不调 libc、不读环境）：注入 `None`
/// 即可确定性单测 `localtime_r`-null 降级路径（无须真 libc 返 null）。
///
...
mod tests {
    ...
    /// 纯函数 `render` 确定性单测——精确值断言，无任何全局 TZ 改动。
    ...
    /// `duration_since` 返 `Err`（墙钟早于 epoch，冷启动 RTC 无电）路径。
    ...
    /// `localtime_r` 返 null（本地时间分解失败）降级路径——经 `compose` 注入 seam
    /// 确定性单测（无须真 libc 返 null）。`None` MUST → 全 0 占位 tm 渲染
    ...
    /// `compose` 的 `Some`（`localtime_r` 成功）路径如实渲染传入的 `tm`，
    ...
    /// 32-bit `time_t`（MIPS post-2038）截断判定——用 `i32` 实例化纯泛型
    ...
    /// `format_log_line` 正常路径产出合法行（带亚秒纳秒 → 毫秒）。
